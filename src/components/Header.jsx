...
const Header = () => {
  ...
  useEffect(() => {
    function handleScrollHighlight() {
      links.forEach((link) => {
        ...
        if (
          scrollTop >= offsetTop &&
          scrollTop < offsetTop + element.offsetHeight
        ) {
  ...
  return (
    <header
      className={`header ${scrollTop > 0 ? "fixed" : ""} ${
        isMobileMenuOpen ? "on" : ""
      }`}
    >
      <div className="fixed_header">
        <div className="inner">
          ...
          <div className="btn_wrap">
            <a href="https://chohyejin53.github.io/" className="btn_blog">
              Blog
            </a>
      ...
      <div className="inner">
        <ul className="gnb">
          {links.map((link) => (
            <li
              key={link.id}
              className={activeMenuItem === link.id ? "on" : ""}
            >
              <a
                href={`#${link.id}`}
